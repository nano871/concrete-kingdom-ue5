...
ACKPedestrianAIController::ACKPedestrianAIController() { BaseSpeed = 60.0f; bFleeing = false; }
...
void ACKPedestrianAIController::ReactToPlayer(float PlayerSpeed, float Distance)
{
    ...
    if (PlayerSpeed > 200.0f && Distance < 500.0f && !bFleeing) {
        // RDR2-style: flee from fast player
        bFleeing = true;
        ...
        GetWorld()->GetTimerManager().SetTimer(UnfleeTimer, [this]() {
            bFleeing = false;
...
void ACKPedestrianAIController::Wander()
{
    ...
    FVector NavLocation;
    if (UNavigationSystemV1::GetCurrent(GetWorld())->GetRandomReachablePointInRadius(WanderOrigin, 500.0f, NavLocation))
